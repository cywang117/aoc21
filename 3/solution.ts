 ...
 * gamma rate and the epsilon rate). The power consumption can then be found by multiplying the gamma rate by the epsilon rate.
 * 
 ...
 */
const partOne = () => {
    const binLen = (input as string[])[0].length;

    // Count sums of '1' and '0' for each binary string index
    const sums = input.join('').split('').reduce((sums, binStr, idx) => {
        if (sums[idx % binLen] < input.length) {
            binStr === '1' && sums[idx % binLen]++;
        }
        return sums;
    }, new Array(binLen).fill(0));

    // Get gamma binary from summed input number count
    const gamma = sums.reduce((gamma, sum) => {
        return gamma + (sum < input.length - sum ? '0' : '1');
    }, '');

    // String bit flip
    const epsilon = gamma.split('').map((b: string) => (1 - Number(b)).toString()).join('');
 ...
 * your diagnostic report - finding them is the tricky part. Both values are located using a similar process 
 * that involves filtering out values until only one remains. Before searching for either rating value, start 
 * with the full list of binary numbers from your diagnostic report and consider just the first bit of those numbers. Then:
 * 
 ...
 */
const partTwo = () => {
    // Split into subarrays of values that start with '0' or '1' bit
    const { ones, zeroes } = input.reduce((acc: { [key: string]: string[] }, binString: string) => {
        binString[0] === '0' ? acc.zeroes.push(binString) : acc.ones.push(binString);
        return acc;
    }, { ones: [], zeroes: [] });

    // Determine which subarrays are o2 and co2
    let [o2, co2] = ones.length >= zeroes.length ? [ones, zeroes] : [zeroes, ones];

    const binaryRadixSort = (a: string, b: string) =>
        parseInt(b, 2) - parseInt(a, 2)

    o2.sort(binaryRadixSort);
    co2.sort(binaryRadixSort);

    const getValue = (array: string[], comparator: (a: number, b: number) => boolean): number => {
        let radix = 1;
        while (array.length > 1 && radix < array[0].length) {
            const sliceIdx = array.findIndex((binString) => binString[radix] === '0');
            array = comparator(sliceIdx, array.length - sliceIdx) ? array.slice(0, sliceIdx) : array.slice(sliceIdx);
            radix++;
        }
        return parseInt(array[0], 2);
    }

    return getValue(o2, (a, b) => a >= b) * getValue(co2, (a, b) => a < b);
}
