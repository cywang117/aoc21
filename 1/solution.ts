 ...
 */
const partOne = () => 
    (input as number[]).reduce((numIncreasing, _, idx, arr) => (
        arr[idx] > arr[idx - 1] && idx !== 0 ? ++numIncreasing : numIncreasing
    ), 0);
 ...
 * marked A (199, 200, 208); their sum is 199 + 200 + 208 = 607. The second window is marked B (200, 208, 210); its sum is 618. 
 ...
 */
const partTwo = () => {
    const numWindowPairs = input.length - 3;
    let [sum1, sum2] = [0, 0];
    let numIncreasing = 0;
    for (let i = 0; i < numWindowPairs; i++) {
        const common = input[i+1] + input[i+2];
        [sum1, sum2] = [input[i] + common, common + input[i+3]];
        sum1 < sum2 && numIncreasing++;
    }
