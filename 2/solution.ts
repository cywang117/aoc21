 ...
 */
const partOne = () => {
    const res = (input as string[]).reduce((position, instruction) => {
        const delta = parseInt(instruction.split(' ')[1]);
        /forward/.test(instruction) && (position.x += delta);
        /up/.test(instruction) && (position.y -= delta);
        /down/.test(instruction) && (position.y += delta);
        return position;
    }, { x: 0, y: 0 });
    return res.x * res.y;
}
...
const partTwo = () => {
    ...
    const res = (input as string[]).reduce((position, instruction) => {
        const delta = parseInt(instruction.split(' ')[1]);
        /down/.test(instruction) && (aim += delta);
        /up/.test(instruction) && (aim -= delta);
        if (/forward/.test(instruction)) {
            position.x += delta;
            position.y += (aim * delta);
        }
        return position;
    }, { x: 0, y: 0 });
    return res.x * res.y;
